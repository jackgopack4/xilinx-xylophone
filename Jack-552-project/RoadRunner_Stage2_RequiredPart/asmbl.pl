...
my %numArgs = ( qw/ADD 3 PADDSB 3 SUB 3 AND 3 NOR 3 SLL 3 SRL 3 SRA 3 LW 3 SW 3 LHB 2 LLB 2 B 2 JAL 1 JR 1 HLT 0/);



my %opcode = ( qw/ADD 0000 PADDSB 0001 SUB 0010 AND 0011 NOR 0100 SLL 0101 SRL 0110 SRA 0111 LW 1000 SW 1001 LHB 1010 LLB 1011 B 1100 JAL 1101 JR 1110 HLT 1111/);
...
while(<IN>) {
  ...
  if( /^\s*(\S+)\s*(.*)/ ) {
      ...
      if($instr =~ /^(AND|NOR|ADD|PADDSB|SUB)$/) {
